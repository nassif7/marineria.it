...
interface IContactCrewInfoModal {
  visible: boolean
  onClose: () => void
  name?: string
...
const ContactRow: FC<ContactEntry & { last?: boolean }> = ({ icon: Icon, label, value, onPress, last }) => (
...
const ContactCrewInfoModal: FC<IContactCrewInfoModal> = ({ visible, onClose, name, entries }) => {
      ...
      <View style={cr.backdrop}>
        ...
        <View style={[cr.card, { marginBottom: Math.max(bottom, 20) }]}>
          <View style={cr.header}>
            <Text style={cr.headerTitle}>{t('contact-crew')}</Text>
          ...
          <View style={cr.memberRow}>
            ...
            <View style={{ flex: 1, minWidth: 0 }}>
              <Text style={cr.memberName}>{name || t('contact-crew')}</Text>
            </View>
          </View>
          ...
          {entries.map((entry, i) => (
            <ContactRow key={entry.label} {...entry} last={i === entries.length - 1} />
          ))}
        </View>
      </View>
    </Modal>
...
export default memo(ContactCrewInfoModal)

ContactCrewInfoModal.displayName = 'ContactCrewInfoModal'
